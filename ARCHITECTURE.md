...
Deeper technical detail for [`README.md`](./README.md) — routes, code layout, the live API's quirks, security posture, and what the test suite covers.
...
- **`slug` can be `null`.** One real job in the dataset (Athyna's own "Senior Full-Stack Engineer" posting, notably one of the very few with `skills` populated) has `slug: null`. The schema treats it as nullable like every other optional field — an earlier, stricter version of the schema silently dropped that job from every response.
...
## Environment variables — the build-time detail

Vite inlines every `VITE_*` variable into the JavaScript bundle when it builds. That means:
